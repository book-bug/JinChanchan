...
class FeatureMatcher:
    ...
    def __init__(self, device='cuda'):
        # 使用ResNet-18作为特征提取器
        self.model = models.resnet18(weights=models.resnet18(weights=models.ResNet18_Weights.DEFAULT))
    ...
    def extract_features(self, img):
        #图像读取与预处理
        img_tensor = self.preprocess(img).unsqueeze_(0).to(self.device)
        #特征提取
        with torch.no_grad():
...
if __name__ == '__main__':
    ...
    #构建特征数据库（只运行一次）
    img_dict = load_imgs(Config.picture_dir)  #  imgs{name:ndarray}
    img_features = {}
    for k, v in img_dict.items():
        img_features[k] = matcher.extract_features(v)

    #执行查询
    img = Image.open(r'D:\python_study\JinChanchan\picture\嘉文四世.png')
